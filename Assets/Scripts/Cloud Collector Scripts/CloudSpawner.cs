...
public class CloudSpawner : MonoBehaviour {
	...
	private float boundsBorder = 0.5f;
	private float temporalY = 0.8f;
	private float collPositionY = 0.7f;
	...
	void SetMinAndMax(){
		...
		minX = -bounds.x + boundsBorder;
		maxX = bounds.x - boundsBorder;
	}

	void Shuffle(GameObject[] toShuffle){
		for(int i=0; i<toShuffle.Length; i++){
			GameObject temp = toShuffle[i];
			int random = Random.Range(i, toShuffle.Length);
